<nav>
    <ul>
        <li><a href="index.php">Accueil</a></li>
        <li><a href="exo1.php">Excercice 1</a></li>
        <li><a href="0-pyramide.php">Excercice 2</a></li>
        <li><a href="1-circle.php">Excercice 3</a></li>
        <li><a href="2-find_the_number.php">Excercice 4</a></li>
        <li><a href="3-calculate_average.php">Excercice 5</a></li>
        <li><a href="4-display_character.php">Excercice 6</a></li>
        <li><a href="5-array.php">Excercice 7</a></li>
        <li><a href="6-info_character.php">Excercice 8</a></li>
        <li><a href="7-associative_arrays.php">Excercice 9</a></li>
        <li><a href="8-array_of_arrays.php">Excercice 10</a></li>
    </ul>
</nav>